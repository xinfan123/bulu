...
public class MessageListView extends ListView implements OnScrollListener, OnItemClickListener {
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == 1) {
				...
				if (addList == null || addList.isEmpty()) {
					Toast.makeText(MessageListView.this.getContext(), "没有更多的数据了...", 0).show();
				} else {
					list.addAll(addList);
				...
				isLoading = false;
				// 移除底部footer
				MessageListView.this.removeFooterView(footer);
				MessageListView.this.setVisibility(View.VISIBLE);
	...
	public void loadData() {
		...
		ad = new RevMessageListAdapter(this.context, list);
	...
	private void getFirstData() {
		...
		new Thread() {
			...
			public void run() {
				...
				List addList = MessageListDataService.getRevMessageData(pageSize, 1);
			...
			if (totalItemCount > 0) {
				...
				new Thread() {
					...
					public void run() {
						...
						List list = MessageListDataService.getRevMessageData(pageSize, page + 1);
