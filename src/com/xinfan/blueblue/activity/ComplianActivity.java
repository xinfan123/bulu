...
public class ComplianActivity extends BaseActivity {
	...
	public void complain_send(View v) {
		...
		if (complainText == null || complainText.equals("") || complainText.trim().length() < 5) {
			ToastUtil.showMessage(ComplianActivity.this, "请先编辑内容！");
			return;
		}

		if (complainText.length() > 500) {
			ToastUtil.showMessage(ComplianActivity.this, "字数过长，不能超过500个字符！");
			return;
		}
		...
		AnsynHttpRequest.requestSimpleByPost(this, request, new RequestSucessCallBack() {
			...
			public void call(Request data) {
				ToastUtil.showMessage(ComplianActivity.this, data.getMessage());
				ComplianActivity.this.finish();
			}
		});
	}
