...
public class Register extends Activity {
	...
	private String mobile, password, nickname, rancode;
		...
		handler = new Handler() {
			public void handleMessage(Message msg) {
				...
				if (message.equals("发送成功")) {
					new AlertDialog.Builder(Register.this).setIcon(getResources().getDrawable(R.drawable.login_editbox)).setMessage("发送成功！请注意查收！").create()
							.show();
				} else {
					new AlertDialog.Builder(Register.this).setIcon(getResources().getDrawable(R.drawable.login_error_icon)).setMessage("发送失败！发生未知错误！").create()
							.show();
				}
			}
	...
	public void register(View v) {
		...
		if (!((p.matcher(mobile)).matches())) {// 判断手机号码格式
			new AlertDialog.Builder(Register.this).setIcon(getResources().getDrawable(R.drawable.login_error_icon)).setMessage("手机号码为空或格式不正确！").create().show();
		} else if (nickname == null || nickname.length() <= 0) {// 判断昵称是否为空
			new AlertDialog.Builder(Register.this).setIcon(getResources().getDrawable(R.drawable.login_error_icon)).setMessage("昵称不能为空！").create().show();
		} else if (password == null || password.length() <= 0) // 判断密码是否为空
		{
			new AlertDialog.Builder(Register.this).setIcon(getResources().getDrawable(R.drawable.login_error_icon)).setMessage("密码不能为空！").create().show();
		} else if (rancode == null || rancode.length() <= 0) {
			new AlertDialog.Builder(Register.this).setIcon(getResources().getDrawable(R.drawable.login_error_icon)).setMessage("验证码不能为空！").create().show();
		} else if (!(rancode.equals(gencode))) {
			new AlertDialog.Builder(Register.this).setIcon(getResources().getDrawable(R.drawable.login_error_icon)).setMessage("验证码不正确！").create().show();
		} else {
